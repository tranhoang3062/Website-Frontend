...
export class IndexOComponent {
    ...
    public constructor(private getService: getService, private orderService: OrderService, private deleteService: DeleteService, private router: Router) {
        this.orderService.getAllOrder(undefined, (err: boolean, dataOrder: any) => {
            this.orderService.getAllOrderDetail(undefined, (err: boolean, dataOrderDetail: any) => {
                if (!err) {
      ...
      <div><input type="radio" name="status" value="6" id="status-6" ${item.status == 6 ? "checked" : ""}><label class="ml-3" for="status-6">Trả hàng</label></div>
            ...
            if (result.isConfirmed) {
                ...
                if (radioElem.value !== item.status) {
                    this.orderService.editOrder(item.id, { status: radioElem.value }, (result: boolean, data: any) => {
                        if (result) {
    ...
    public handleDelete(item: any, index: number) {
        this.deleteService.onDelete(undefined, '', (result: boolean) => {
            if (result) {
                this.orderService.deleteOrderDetailByOrderId(item.id, (result: boolean, data: any) => {
                    if (result) {

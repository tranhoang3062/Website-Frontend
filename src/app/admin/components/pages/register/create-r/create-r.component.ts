...
export class CreateRComponent {
  ...
  public constructor(private fb:FormBuilder, private router:Router, private getService:getService, private createService:CreateService, private activatedRoute:ActivatedRoute) {
            ...
            this.getService.getAllUser('?phone=' + newData.phone, (err:boolean, data:any) => {
              if(!err) {
                if(data[0]) this.phoneError = true;
                  ...
                  this.createService.register(formData, (result:boolean) => {
                    if(result) {

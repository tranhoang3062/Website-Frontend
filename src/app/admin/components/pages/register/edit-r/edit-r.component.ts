...
export class EditRComponent {
  ...
  public constructor(private fb:FormBuilder, private router:Router, private getService:getService, private editService:EditService, private activatedRoute:ActivatedRoute) {
    this.activatedRoute.paramMap.subscribe(params => {
      this.paramType = params.get('type');
      this.paramId = params.get('id');
      this.getService.getUserById(this.paramId, (err:boolean, data:any) => {
        if(!err) {
          this.account = data[0];
  ...
  public handleSubmit() {
    ...
    this.getService.getAllUser('?email=' + newData.email, (err:boolean, data:any) => {
      if(!err) {
        if(data[0] && data[0].email !== newData.email) this.emailError = true;
        else {
          this.emailError = false;
          this.getService.getAllUser('?phone=' + newData.phone, (err:boolean, data:any) => {
            if(!err) {
              if(data[0] && data[0].phone !== newData.phone) this.phoneError = true;
                ...
                this.editService.editUser(this.paramId, formData, (result:boolean) => {
                  if(result) {
                    Swal.fire({

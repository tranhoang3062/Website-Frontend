    ...
    public constructor(
        ...
        this.activatedRoute.queryParamMap.subscribe(params => {
            ...
            if (!this.queryName) {
                ...
                this.categoryService.getSearchCate(this.queryName, (err: boolean, data: any) => {
                    if (!err) {
                        this.dataCategoryDMTC = data.filter((item: any) => item.parent_id == 0);
                        this.dataCategoryDMSP = data.filter((item: any) => item.parent_id != 0);
                    }
                });
                this.petService.getSearchPetlist(this.queryName, (err: boolean, data: any) => {
                    if (!err) {
                        this.dataListpet = data;
                    }
                });
        ...
        this.deleteService.onDelete(undefined, "Danh mục có thể chứa dữ liệu quan trọng", (result: boolean) => {
            if (result) {
                if (key === 'dmtc' || key === 'dmsp') {
                    this.categoryService.deleteCategory(id, (result: boolean) => {
                        if (result) {
                    ...
                    this.petService.deletePetlist(id, (result: boolean) => {
                        if (result) {

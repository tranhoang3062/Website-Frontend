...
export class EditCtComponent {
  ...
  public constructor(private fb:FormBuilder, private router:Router, private getService:getService, private activatedRoute:ActivatedRoute, private editService:EditService, private categoryService:CategoryService) {
  ...
  public handleSubmit() {
    ...
    if(this.paramsKey !== "gtc") {
      ...
      if(!this.nameError) {
        ...
        this.editService.editCategory(this.paramsId, newData, (result:boolean) => {
          if(result) {
      ...
      if(!this.nameError) {
        newData.status = this.radioInputs.find((item:any) => item.checked).value;
        this.editService.editPetlist(this.paramsId, newData, (result:boolean) => {
          if(result) {
            this.router.navigateByUrl('/admin', { skipLocationChange: true }).then(() => {
              this.router.navigate(['/admin/category'], {queryParams:oldQuery});
            });

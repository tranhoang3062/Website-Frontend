...
export class CreateCtComponent {
  ...
  public constructor(private fb:FormBuilder, private router:Router, private getService:getService, private createService:CreateService, private activatedRoute:ActivatedRoute, private categoryService:CategoryService) {
  ...
  public handleSubmit() {
    ...
    if(this.paramsKey !== "gtc") {
      ...
      if(!this.nameError) {
        this.createService.createCate(newData, (result:boolean) => {
          if(result) {
            if(this.paramsKey === "dmtc") {
      ...
      if(!this.nameError) {
        this.createService.createPetList(newData, (result:boolean) => {
          if(result) {
            this.router.navigateByUrl('/admin', { skipLocationChange: true }).then(() => {

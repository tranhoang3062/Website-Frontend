...
export class EditNComponent {
  ...
  public constructor(private fb:FormBuilder, private router:Router, private getService:getService, private editService:EditService, private activatedRoute:ActivatedRoute, private categoryService:CategoryService) {
  ...
  public handleSubmit() {
    ...
    if(!this.titleError && !this.slugError) {
      ...
      this.editService.editNews(this.paramId, formData, (result:boolean) => {
        if(result) {

...
export class DetailProductComponent {
  ...
  public constructor(private titleService:Title, private getService:getService, private activatedRoute:ActivatedRoute, public productService:ProductService, public wishlistService:WishlistService, private fb:FormBuilder, private createService:CreateService, private router:Router, private editService:EditService) {
      ...
      this.getService.getAllProducts('?slug='+this.paramName, (err:boolean, data:any) => {
        if(!err) {
          if(data[0]) {
            ...
            this.getService.getCommentByProduct(this.product.id, (err:boolean, data:any) => {
              if(!err) {
  ...
  public handleSubmit() {
    if(this.user) {
      ...
      if(!this.yourComment) {
        ...
        this.createService.createComments(formData, (result:boolean, res:any) => {
          if(result) {
            this.totalCmt += 1;
        ...
        this.editService.editComments(this.yourComment.id, formData, (result:boolean, res:any) => {
          if(result) {

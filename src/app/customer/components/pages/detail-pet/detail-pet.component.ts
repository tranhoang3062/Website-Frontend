...
export class DetailPetComponent {
  ...
  public constructor(private getService:getService, private activatedRoute:ActivatedRoute, public productService:ProductService, private titleService:Title) {
    ...
    this.activatedRoute.paramMap.subscribe(params => {
      ...
      this.getService.getAllPets('?slug='+this.paramName, (err:boolean, data:any) => {
        if(!err) {
          this.pet = {...data[0], resources: JSON.parse(data[0].resources)};
          this.pet.description = this.pet.description.replace('h2>', 'h4>');
          this.getService.getAllPets('?pet_list_id='+this.pet.pet_list_id, (err:boolean, data:any) => {
            if(!err) {
